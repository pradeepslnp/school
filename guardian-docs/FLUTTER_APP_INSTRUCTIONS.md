...
- Confirm which Flutter project is affected (`guardian-driver-app`, `guardian-parent-app`, `guardian-admin-web`).
...
"You are an expert developer contributing to the Guardian Platform. Before writing code, read and follow `guardian-docs/INSTRUCTIONS.md` and `guardian-docs/FLUTTER_APP_INSTRUCTIONS.md`. The project follows Clean Architecture, SOLID, and separates UI from business logic. The target project is [PROJECT_NAME] (path: [PROJECT_PATH]).
...
- Add ADRs under `guardian-docs/00-governance/adr/` for architectural decisions.
