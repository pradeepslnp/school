...
docker compose -f guardian-infra/docker-compose.yml ps
```
...
`guardian-infra/db/init/01-roles.sql` runs on first start and creates the two roles that make RLS meaningful ([`RLS_POLICIES.md`](../03-database/RLS_POLICIES.md)):
...
A local RSA keypair is generated on first run into `guardian-backend/.local/` (gitignored) — no shared development signing key.
...
cd ../guardian-parent-app && flutter pub get && flutter run
cd ../guardian-driver-app && flutter pub get && flutter run
cd ../guardian-admin-web  && flutter pub get && flutter run -d chrome
```
...
The architecture tests read `BUSINESS_RULES.md` and `PERMISSION_MATRIX.md` out of `guardian-docs`, resolved at `../../guardian-docs`. If that repository is checked out elsewhere, point at it explicitly rather than copying the files:

```bash
GUARDIAN_DOCS_PATH=/path/to/guardian-docs ./gradlew test
```
...
```bash
cd guardian-parent-app && flutter test
```
...
```bash
docker compose -f guardian-infra/docker-compose.yml down -v
docker compose -f guardian-infra/docker-compose.yml up -d
```
...
```bash
docker compose -f guardian-infra/docker-compose.yml logs -f postgres
docker compose -f guardian-infra/docker-compose.yml exec postgres psql -U guardian_owner -d guardian
...
Then open [`guardian-tenancy`](../../guardian-backend/guardian-tenancy/) — it is the reference implementation, and every other module follows it.
