...
- [ ] Documentation updated — `documentation` PR open and linked, merged first
...
| New table | `documentation/03-database/tables/`, migration **with RLS** |
...
All of these live in `documentation`, so documentation can no longer land in the same PR as the code. It still lands in the same **change**: open the `documentation` PR first, link it from the code PR, and merge docs first. Neither merges alone.
