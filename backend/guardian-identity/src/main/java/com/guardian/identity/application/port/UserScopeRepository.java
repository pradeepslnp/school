 ...
 *
 * <p>Every method runs inside an already-established tenant context — ordinary row-level security,
 * no {@code SECURITY DEFINER} bypass — matching {@link RoleProvisioningPort}, which this is always
 * called alongside: a role grant with no accompanying scope answers "what" without "which"
 * (BR-IAM-006).
 */
public interface UserScopeRepository {

  /**
   * Every scope currently held by {@code userId}, for presenting on login (BR-IAM-001, affordance
   * only).
   */
