...
///
/// In this app a failed request is normal, not exceptional: vehicles lose signal on a route.
/// Nothing here throws for a network fault — every failure becomes an
/// [ApiResponse.transportFailure] so the caller can queue the event locally and move on
/// (ADR-0008). A boarding record must never be lost because a socket dropped.
///
...
class RestClient {
  ...
  /// Identifies the calling app to the server: `DRIVER_APP`.
  ...
  /// Times each exchange so the caller can measure clock skew against the server's `Date`
  /// header (ADR-0008). Injected so a test can produce a deterministic round trip.
  final Clock _clock;
  ...
  Future<ApiResponse> postMultipart(
    ...
    try {
      ...
      // An upload takes long enough that its round trip is worthless as a skew measurement;
      // the timings are still recorded, and the estimator discards windows this wide.
      return _toApiResponse(
        response,
        sentAt: sentAt,
        receivedAt: _clock.nowUtc(),
      );
    } on Object {
  ...
  Future<ApiResponse> _send(
    ...
    for (var attempt = 0; attempt < attempts; attempt++) {
      try {
        ...
        // Bracketed as tightly as possible around the single attempt: a window that also
        // covered backoff or an earlier attempt would put the skew estimate out by seconds.
        final sentAt = _clock.nowUtc();
        final response =
        ...
        return _toApiResponse(response, sentAt: sentAt, receivedAt: receivedAt);
      } on SocketException {
